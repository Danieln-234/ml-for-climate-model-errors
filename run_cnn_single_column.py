...
def plot_ig_summary(
    ...
    for c in range(C):
        ...
        ax1.fill_between(level_values, mean_ig[c, :] - std_ig[c, :], mean_ig[c, :] + std_ig[c, :], alpha=0.3, label="±1σ for ONLY each of the 20 runs")
        ax1.set_xlabel("Pressure level")
        ax1.set_ylabel("Integrated Gradients", color="blue")
        ax1.tick_params(axis="y", labelcolor="blue")
        ax1.invert_xaxis()  # low pressure right side- liek height TODO mabybe make log
...
if __name__ == "__main__":
    ...
    "Multiple columns"
    "Take out stability at some point maybe -this was note to self"
    "Ploit std bounds instead of stability (or percentile shading)"
    "eXPLICITLY NAME THE COLUMN in graphs"
